...
//
//
//  Created by Roberto Oliveira on 24/07/17.
...
open class BackButton: UIButton {
    ...
    open var iconPathInset:CGFloat = 4.0
    open var iconPosition:CGPoint = CGPoint(x: -2, y: 0)
    ...
    open override func layoutSubviews() {
        ...
        self.shape.path = self.backPath().cgPath
        self.shape.frame = self.bounds
        self.shape.frame.origin = self.iconPosition
    ...
    // MARK: - Path Method
    private func backPath() -> UIBezierPath {
        let rect:CGRect = self.bounds
        let path = UIBezierPath()
        let pathInset:CGFloat = self.iconPathInset
        path.move(to: CGPoint(x: ((rect.size.width*0.75)-pathInset), y: (pathInset*2.0)))
        path.addLine(to: CGPoint(x: ((rect.size.width*0.25)+pathInset), y: (rect.size.height/2)))
        path.addLine(to: CGPoint(x: ((rect.size.width*0.75)-pathInset), y: (rect.size.height-(pathInset*2.0))))
        return path
    }
